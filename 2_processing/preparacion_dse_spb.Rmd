...
```{r}
sum(is.na(elsoc_mod))
colSums(is.na(elsoc_mod))
aggr_plot <- aggr(elsoc_mod,numbers=TRUE,sortVar=TRUE, labels= names(elsoc_mod), cex.axis=.7, gap=3, ylab=c("Histogram of missing data","Pattern"))
```
...
elsoc_clean <- mutate(elsoc_clean,PERCEPCION_SEGURIDAD = car::recode(elsoc_clean$PERCEP_SEGURIDAD,"1 = 5; 2 = 4; 3 = 3; 4 = 2; 5 = 1"))

#finalmente construimos el índice de "Conflictividad Meso-Social"
CONFLIC_MESO <-((elsoc_clean$EXP_CRIMINALIDAD + elsoc_clean$PERCEP_SEGURIDAD)/4)
elsoc_clean$CONFLIC_MESO=CONFLIC_MESO #lo añadimos al dataframe
...
elsoc_clean$CALIDAD_RSBARRIO=CALIDAD_RSBARRIO #añadimos una nueva columna "COHESION" al dataframe

##----construimos un índice de "Sentido de pertenencia"
SENTIDO_PERTENENCIA <- ((elsoc_clean$BARRIO_IDEAL + elsoc_clean$INTEGRADO_BARRIO + elsoc_clean$IDENTIFICADO_BARRIO + elsoc_clean$PERTENENCIA_BARRIO + elsoc_clean$CONFIANZA_VECINOS)/5)
elsoc_clean$SENTIDO_PERTENENCIA=SENTIDO_PERTENENCIA #añadimos una nueva columna "COHESION_BARRIAL" al dataframe
...
tabla_indice_meso <- dplyr::select(elsoc_clean,PERCEPCION_SEGURIDAD,FREC_PELEAS,FREC_ROBOS,FREC_TRAFICO)
...
TASA_HACIN <-(elsoc_clean$HACINAMIENTO/elsoc_clean$NVIVIENDAS)
elsoc_clean$TASA_HACIN=TASA_HACIN #añadimos una nueva columna
```
...
elsoc_barrio <- dplyr::select(
  ...
  sentido_pertenencia=SENTIDO_PERTENENCIA,conflicto_meso=CONFLIC_MESO,conflicto_micro=CONFLIC_MICRO,calidad_rrss=CALIDAD_RSBARRIO,
  rep_barrial=REPUTACION_BARRIAL,satisf_barrial=SATISF_BARRIAL,tam_viv=TAMANO_VIV,calidad_viv=CALIDAD_VIV,
  servicios_publicos=SERVICIOS_PUBLICOS,densidad=DENSIDAD.RS, puntaje_violencia=SCORE_VIOLENCIA,
  puntaje_sustraccion=SCORE_SUSTRACCION,tasa_hacinamiento=TASA_HACIN, nvl_socioecon=NVLSOEC.RS, expo_hete=EXPO_HETE)
