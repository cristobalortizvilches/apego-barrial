...
elsoc_mod$sentido_pertenencia <- ((elsoc_mod$barrio_ideal + elsoc_mod$integrado_barrio + elsoc_mod$identificado_barrio + elsoc_mod$pertenencia_barrio)/4)
elsoc_mod$sentido_pertenencia <- set_label(x = elsoc_mod$sentido_pertenencia, label = "Sentido de pertenencia al barrio")

#-----Construcción X1: Experiencia de Habitar el Territorio----

##----índice de "Percepción de calidad en relaciones sociales barriales" 
elsoc_mod$calidad_rrss <- ((elsoc_mod$amistad + elsoc_mod$sociabilidad + elsoc_mod$cordialidad + elsoc_mod$colaboracion)/4)
elsoc_mod$calidad_rrss <- set_label(x = elsoc_mod$calidad_rrss,label = "Percepción de calidad de relaciones sociales en el barrio")
...
elsoc_mod$exp_criminalidad <- ((elsoc_mod$frec_peleas + elsoc_mod$frec_robos + elsoc_mod$frec_trafico)/3)
elsoc_mod$exp_criminalidad <- set_label(x = elsoc_mod$exp_criminalidad,label = "Experiencias de criminalidad en el barrio")

##----índice de "Nivel de inseguridad experimentada en el barrio"
elsoc_mod$nvl_inseguridad <- ((elsoc_mod$sent_inseguridad + elsoc_mod$insatisf_seguridad + elsoc_mod$frec_peleas + elsoc_mod$frec_robos + elsoc_mod$frec_trafico)/5)
...
elsoc_mod$participacion <- car::recode(elsoc_mod$participacion,"1=0;2=1;3=2")
elsoc_mod$participacion <- as.factor(elsoc_mod$participacion)
...
tabla_indice_crrss <- dplyr::select(elsoc_mod,amistad,sociabilidad,cordialidad,colaboracion)
...
tabla_indice_expcrim <- dplyr::select(elsoc_mod,frec_peleas,frec_robos,frec_trafico)
#cronbach(tabla_indice_expcrim) #0.75

tabla_indice_nvlinseguridad <- dplyr::select(elsoc_mod,sent_inseguridad,insatisf_seguridad,frec_peleas,frec_robos,frec_trafico)
...
elsoc_barrio <- dplyr::select(
  elsoc_mod,sexo, edad, participacion, #variables sociodemográficas
