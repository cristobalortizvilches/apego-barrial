...
elsoc_gs <- elsoc_cit_2016 %>% 
  ...
  dplyr::select(idencuesta, #id individual
                ...
                nived_zon, sded_zon, dviol_zon, dens_zon, accs_zon, #variables cit zona
                nived_dto, sded_dto, dviol_dto, dens_dto, accs_dto, #variables cit distrito
...
elsoc_gs <- elsoc_gs %>% 
  ...
  ungroup() %>% 
  mutate(nive_zon = rescale(nived_zon, to = c(1,5)),
         sded_zon = rescale(sded_zon, to = c(1,5)),
         viol_zon = rescale(dviol_zon, to = c(1,5)),
         dens_zon = rescale(dens_zon, to = c(1,5)), 
         accs_zon = rescale(accs_zon, to = c(1,5)), 
         nive_dto = rescale(nived_dto, to = c(1,5)),
         sded_dto = rescale(sded_dto, to = c(1,5)),
         viol_dto = rescale(dviol_dto, to = c(1,5)), 
...
elsoc_entorno <- elsoc_gs %>% 
  select(idencuesta, estrato, comuna.x, comuna_cod, manzana, zona, distrito,
         apbi, apbr, soci, cnfi, segu, acci, geni, desi, repb, time,
         repb_zon, nive_zon, sded_zon, viol_zon, dens_zon, accs_zon,
         repb_dto, nive_dto, sded_dto, viol_dto, dens_dto, accs_dto,
         nived_com)
